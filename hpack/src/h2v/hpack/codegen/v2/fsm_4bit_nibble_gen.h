...
//   - kNibbleDecodeTable[512*16]    (packed 32-bit entries, exactly 32,768
//   bytes (32KB))
...
static void EmitNibbleTables(std::ofstream& out,
  ...
  out << "//       IF THIS FILE MISMATCH, HUFFMAN DECODE WILL FAIL.\n";
  ...
  out << "// Packed nibble decode table: 512 states x 16 nibbles = 8192 "
         "entries → 32,768 bytes (32KB).\n";
  out << "// Pack into 32 bits:\n"
         "//  bit  [31    ] = 0\n"
         "//  bits [30..22] = next_state (9 bits)\n"
         "//  bits [21..20] = emit_count (2 bits)\n"
         "//  bits [19..12] = s0 (8 bits)\n"
         "//  bits [11..4 ] = s1 (8 bits)\n"
         "//  bits [3..0  ] = 0 (unused)\n";
  ...
  // -------------------------------------------------------------------------
  {
    ...
    out << "// Accepting bits for nibble-FSM (513 bits) → stored as "
           "9x64-bit = 72 bytes\n";
  ...
  // -------------------------------------------------------------------------
  out << "// --- Bit table for nibble-FSM (512 states x 2 bits x 2 bytes = "
         "2,048 bytes (2KB).\n";
...
// -----------------------------------------------------------------------------
inline int GenerateFsm4BitNibble(const std::string& out_path) {
