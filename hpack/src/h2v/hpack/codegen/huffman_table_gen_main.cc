...
int main(int argc, char** argv) {
  ...
  // Build Huffman trie (257 symbols)
  Node* root = new Node();
  ...
  out << "// @generated by h2v_huffman_gen\n"
         ...
         "namespace hpack{\n"
         "namespace huffman{\n"
         "struct ByteDecodeEntry { uint16_t next_state; uint8_t emit_count; "
         "uint8_t symbols[2]; };\n";

  size_t total = nodes.size() * 256;
  out << "static const ByteDecodeEntry kByteDecodeTable[" << total << "] = {\n";

  // Emit each decoded (state, byte) entry
  ...
  out << "static const bool kAccepting[" << nodes.size() << "] = {\n";
  ...
  // Emit the encode table
  out << "\n// Encode table: pre-packed Huffman codes per symbol\n"
         "struct EncodeEntry { uint8_t byte_count; uint8_t bytes[5]; };\n"
         "static const EncodeEntry kEncodeTable[257] = {\n";

  for (int sym = 0; sym < 257; ++sym) {
    uint32_t code = CODE[sym];
    uint8_t length = LEN[sym];
    // Align code to the MSB of a 32-bit word
    uint64_t bits = uint64_t(code) << (64 - length);
    // Pad the remaining bits with 1s
    if (length & 7) {
      bits |= (uint64_t(1) << (64 - length)) - 1;
    }
    int byte_count = (length + 7) / 8;
    out << "  { " << byte_count << ", { ";
    for (int b = 0; b < byte_count; ++b) {
      uint8_t byte = bits >> (56 - 8 * b);
      out << "0x" << std::hex << int(byte) << std::dec;
      if (b + 1 < byte_count)
        out << ", ";
    }
    // pad to 5 slots
    for (int b = byte_count; b < 5; ++b)
      out << ", 0x00";
    out << " } },\n";
  }
  out << "};\n";
